...
exact_IR_loss(x,I,R,t0,t1,b,c,s0,i0) = exact_I_loss(x,I,t0,b,c,s0,i0) + exact_R_loss(x,R,t1,b,c,s0,i0)


function fit_exact_IR_without_delay(x,I,R)
    i0 = I[1]
    # γ = b-c
    # R0 = b/c
    # b = γ * R0/(1-R0)
...
function fit_exact_IR_with_delay(x,I,R)
    i0 = I[1]
    # γ = b-c
    # R0 = b/c
    # b = γ * R0/(1-R0)
    # c = γ /(R0 - 1)
    F((s0,t0,t1,γ,R0)) = exact_IR_loss(x,I,R,t0,t0+t1,γ*R0/(R0-1),γ/(R0-1),s0,i0)
