...
# April
# Dates =       [01.4,02.4,03.4,04.4,05.4,06.4,07.4,08.4,09.4,10.4,11.4,12.4,13.4,14.4,15.4,16.4,17.4, 18.4 19.4, 20.4, 21.4, 22.4]
push!(Infected, [8900,9060,9193,8990,8705,8523,8230,8067,7603,7070,6835,6557,6288,6142,5830,5055,4418, 3980, 3743, 3654, 3348, 3051]...)
push!(Recovered,[1436,1749,2022,2507,2998,3463,4046,4512,5240,6064,6604,6987,7343,7633,8098,8986,9704,10214,10501,10631,10971,11328]...)
push!(Dead,     [ 146, 158, 168, 168, 204, 220, 243, 273, 295, 319, 337, 350, 368, 384, 393, 419, 431,  443,  452,  470,  491,  510]...)
...
for _ in 1:100
    A = rand(100).*5
    b = sqrt(sum(A.^2)) <= sqrt(100)*sum(abs.(A))
    # push!(bs, b)

    b2 = maximum(abs.(A))^2 == maximum(A.^2)
    push!(bs, b2)
end
